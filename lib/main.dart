            ...
            builder: (context, appState, _) => Column(
              ...
              children: [
                ...
                if (appState.loggedIn) ...[
                  ...
                  GuestBook(
                    deleteMessage: (id) => appState.deleteMessageInGuestBook(id),
                    addMessage: (message) =>
...
class ApplicationState extends ChangeNotifier {
        ...
        _guestBookSubscription = FirebaseFirestore.instance
          ...
          for (final document in snapshot.docs) {
            _guestBookMessages.add(
              GuestBookMessage(
                mid: document.data()['messageId'] as String,
                datetime: document.data()['timestamp'] as int,
  ...
  Future<void> deleteMessageInGuestBook(String id) {
    if (!_loggedIn) {
      throw Exception('Must be logged in');
    }
    return FirebaseFirestore.instance
        .collection('guestbook').doc(id).delete();
  }
}

class GuestBookMessage {
  GuestBookMessage(  {required this.datetime,  required this.uid, required this.mid, required this.docId, required this.name, required this.message});
  final String name;
  final String message;
  final int datetime;
  final String uid; //user id (who wrote?)
...
class GuestBook extends StatefulWidget {
  const GuestBook({
    ...
    required this.messages, required this.deleteMessage,
  });
  final FutureOr<void> Function(String id) deleteMessage;
            ...
            children: [
              Flexible(
                child: Column(
                  ...
                  children: [
                  Flexible(child: Paragraph('${message.name}: ${message.message}')),
                  Text(DateTime.fromMillisecondsSinceEpoch(message.datetime, isUtc: false).toString()),
                ],),
              ),
              FirebaseAuth.instance.currentUser!.uid == message.uid ?
              IconButton(icon: const Icon(Icons.delete),onPressed: () async {
                await widget.deleteMessage(message.docId);
              },) : Container(),
            ],
          ),
        const SizedBox(height: 8),
